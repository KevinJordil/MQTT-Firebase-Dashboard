  ...
  methods: {
    async createAutomaton() {
      fire
        ...
        .then(result => {
          console.log(result.data.automatonId);
          this.$dialog.alert({
            title: "Votre automate a été créé",
            message: `Donnée de connexion pour l'automate :<br><strong>username</strong>${
              this.name
            }<br><strong>password : </strong>${result.data.automatonId}`,
